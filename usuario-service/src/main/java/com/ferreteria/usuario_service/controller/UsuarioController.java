...
public class UsuarioController {
    ...
    public ResponseEntity<CollectionModel<EntityModel<Usuario>>> obtenerTodos() {
        ...
        List<EntityModel<Usuario>> usuariosModel = usuarios.stream()
                .map(assembler::toModel) // <--- Usamos el assembler
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Usuario localizado correctamente", content = @Content(mediaType = "application/json", schema = @Schema(implementation = Usuario.class))),
    ...
    public ResponseEntity<EntityModel<Usuario>> obtenerPorId(@PathVariable Long id) {
        ...
        return ResponseEntity.ok(assembler.toModel(usuario)); // <--- Usamos el assembler
    }
